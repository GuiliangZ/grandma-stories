...
每次打开都先「请问您是哪一位？」（选人 / 新建名字），之后所有录音归到这个人名下，首页右上角「换人」可切换。
**任何手机**打开同一个链接，选同一个名字，都能看到这个人之前的录音和文字（都从 Mac 取）。

**密码（可选，默认没有）**：每个人进去后首页有「🔒 设置密码」，设了以后任何手机选这个人都要输对密码才能进她的故事页、听她的录音；
不设就直接进。密码只在本次打开有效，关掉再开要重新输，共用一部手机也安全。再点一次可以改密码，留空按「好了」就是取消密码。
忘了密码在 Mac 上：`python3 server/manage.py set-password 奶奶 1234` 或 `clear-password 奶奶`。家人页的口令仍然能看到所有人（管理员用）。
...
python3 server/manage.py set-password 奶奶 1234  # 重设密码；clear-password 奶奶 取消；clear-passwords 全部取消
